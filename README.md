...
**Note:** Merged upstream FunAudioLLM/CosyVoice into this repository on 2025-10-20. Upstream README retained.
...
for i, j in enumerate(cosyvoice.inference_zero_shot('收到好友從遠方寄來的生日禮物，那份意外的驚喜與深深的祝福讓我心中充滿了甜蜜的快樂，笑容如花兒般綻放。', '', '', zero_shot_spk_id='my_zero_shot_spk', stream=False)):
    torchaudio.save('zero_shot_{}.wav'.format(i), j['tts_speech'], cosyvoice.sample_rate)
cosyvoice.save_spkinfo()

# fine grained control, for supported control, check cosyvoice/tokenizer/tokenizer.py#L248
for i, j in enumerate(cosyvoice.inference_cross_lingual('在他講述那个荒诞故事的过程中，他突然[laughter]停下来，因为他自己也被逗笑了[laughter]。', prompt_speech_16k, stream=False)):
    torchaudio.save('fine_grained_control_{}.wav'.format(i), j['tts_speech'], cosyvoice.sample_rate)

# instruct usage
for i, j in enumerate(cosyvoice.inference_instruct2('收到好友從遠方寄來的生日禮物，那份意外的驚喜與深深的祝福讓我心中充滿了甜蜜的快樂，笑容如花兒般綻放。', '用四川話說這句話', prompt_speech_16k, stream=False)):
...
def text_generator():
    ...
    yield '讓我心中充滿了甜蜜的快樂，'
    yield '笑容如花兒般綻放。'
for i, j in enumerate(cosyvoice.inference_zero_shot(text_generator(), '希望你以後能夠做的比我還好呦。', prompt_speech_16k, stream=False)):
    torchaudio.save('zero_shot_{}.wav'.format(i), j['tts_speech'], cosyvoice.sample_rate)
```
...
for i, j in enumerate(cosyvoice.inference_sft('你好，我是通義生成式語音大模型，請問有什麼可以幫您的吗？', '中文女', stream=False)):
